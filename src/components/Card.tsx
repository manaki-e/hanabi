...
export default function Card({
  ...
  return (
    <div>
      <div
        className={`flex w-28 flex-col justify-between ${hint ? 'rounded-t-xl' : 'rounded-xl'} bg-gray-700 text-center align-middle`}
      >
        <Hanabi color={text_color}></Hanabi>
        <p className={`${text_color} py-2 font-mono text-3xl`}>{number ?? '?'}</p>
      </div>
      {hint && (
        <div className="flex w-28 justify-between rounded-b-xl border-t bg-gray-700 px-4 text-center align-middle">
